...
*/
/*socket.on(strGroupSocket,  function(data){ // canal por cada grupo !

})*/


socket.emit('subscribe', "58d1a19621b3e007f4230909")
//socket.emit('subscribe', "sendDataGroup")

/*socket.emit('subscribe', "my-namespace")
...
//socket.to('sendDataGroup').emit("holaaaaaaaa")
socket.emit('sendDataGroup', {room:'58d1a19621b3e007f4230909', message: "mensaje para el grupo  EXCLUSIVOOO sendDataGroup"})

socket.on('groupSocketIO', function(data){ // canal por cada grupo !
    console.log("groupSocketIO socket ");
    data = JSON.parse(data)
    console.log( " CLIENT groupSocketIO -------" + data);
    console.log( " CLIENT groupSocketIO  MESSAGE -------" + data.messageTo)

})


var myGroups = new Array()
myGroups.push('58d1a19621b3e007f4230909')

socket.on('58d1a19621b3e007f4230909'||'58d1a19621b3e007f4230909', function(data){ // canal por cada grupo !
    //socket.emit('58d1a19621b3e007f4230909', args)
...
socket.on('sendDataGroup', function(data){ // canal por cada grupo !
    //socket.emit('sendDataGroup', args)
    console.log("client socket  sendDataGroup");
    //data = JSON.parse(data)
    console.log( " Cliente ------- 'sendDataGroup'  =>  " +  data);
...
socket.emit('sendDataGroup', {room:'58d1a19621b3e007f4230909', message: "Mensaje para el grupo  EXCLUSIVOOO "})

socket.on('message', function(data){ // TODO usar para saber el room y
    console.log(" ########### DATA de socket 'message' ",   data);
})
...
var message = {
    room : '58d1a19621b3e007f4230909',
    message : 'Mensaje desde el cliente de l grupp   58d1a19621b3e007f4230909 GRUPO'
}
...
message['message'] = $('#groupMessage').val();

message = JSON.stringify(message); // FIXME, si se va a ajax da error en parseo (volver a definir tood el mensaje  ? )


$('#sendMessage').click(function() {
    //socket.emit('sendDataGroup', {room:'58d1a19621b3e007f4230909', message: "mensaje para el grupo  EXCLUSIVOOO sendDataGroup"})
    console.log("Ajaxxxxxxxxxxxxxxxxxxxxxxx ", (message));
    //socket.emit('58d1a19621b3e007f4230909', JSON.stringify(message))
    //socket.emit('58d1a19621b3e007f4230909', (message))

    $.ajax({'url': BASE_URL + '/groups/notif/' + '58d1a19621b3e007f4230909', // FIXME
        'type' : 'POST',
        //'json': true,
        'headers' : {'Content-Type' : 'application/json'},
        'data' :  message, //JSON.stringify({'groupMessage': $("#groupMessage").val()}) ,  //JSON.stringify(requestObject), // datos a enviar
        'success' : function(data){
            //console.log(" Sucess data <rq /groups/notif/ +,",  message.room.toString(), "  ", data );
        },
